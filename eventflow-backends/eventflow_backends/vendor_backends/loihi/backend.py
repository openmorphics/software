...
class LoihiBackend:
    ...
    def run(
        ...
        """
        if not _check_nxsdk():
            raise RuntimeError("NxSDK not available. Cannot run on Loihi hardware.")

        if plan is None:
            plan = self.plan(eir)

        # For now, stub implementation - in real impl, would compile to NxSDK and run
        _log.warning("Loihi backend is a stub implementation. Hardware execution not implemented.")
