...
class SpiNNakerBackend:
    ...
    def run(
        ...
        """
        if not _check_spynnaker():
            raise RuntimeError("sPyNNaker not available. Cannot run on SpiNNaker hardware.")

        if plan is None:
            plan = self.plan(eir)

        # For now, stub implementation - in real impl, would compile to sPyNNaker and run
        _log.warning("SpiNNaker backend is a stub implementation. Hardware execution not implemented.")
