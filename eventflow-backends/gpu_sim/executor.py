"""
gpu-sim backend executor v0.1

Provides:
- plan_gpu_sim(eir, dcd) -> plan dict
- run_gpu_sim(plan, inputs_jsonl, out_trace_path) -> run result dict

This is a deterministic, minimal executor mirroring cpu-sim behavior but scoped
for a GPU-simulator backend. It focuses on canonical ordering and trace capture.
"""
...
def plan_gpu_sim(eir: Dict[str, Any], dcd: Dict[str, Any]) -> Dict[str, Any]:
    """
    Capability-aware planning v0.1 (GPU simulator):
    - Enforce profile compatibility (fatal)
    - Enforce epsilon contracts vs device time resolution
    - Quantize fixed_step dt to device resolution
    - Report emulated ops
    """
    ...
    # Time mode support (warn + emulate)
    if mode not in deterministic_modes or (mode == "exact_event" and fixed_only):
        warnings.append(f"backend does not support time.mode='{mode}' deterministically; will emulate")

    # Time negotiation and epsilon checks
    ...
    plan = {
        "backend": {"name": "gpu-sim", "version": dcd.get("version", "0.1.0"), "mode": mode},
        "graph": {
        ...
        "notes": "gpu-sim plan (deterministic executor with capability negotiation)",
    }
    return plan


def run_gpu_sim(plan: Dict[str, Any], inputs: List[str], out_trace_path: str) -> Dict[str, Any]:
    """
    Merge input Event Tensor JSONL streams by canonical order and emit a trace.
    This mirrors the cpu-sim baseline and keeps deterministic semantics.
    """
    # Read inputs
